    ...
    def load_dwd_warnings(self, kwargs):
        ...
        if r.status_code == 200:
            ...
            for warning in data_sorted:
                ...
                sensor_name = "sensor.dwd_warn_" + event.lower().replace(" ","_") + "_" + warning[1].replace("-","_").replace(":","_").replace("T","_").replace("Z","")
                if (self.get_state(sensor_name) != event) or (self.get_state(sensor_name, attribute = "Gefahr (0-4)") != warning[0]):
                    ...
                    if warning[0] >= 1: # Severity
                        self.fire_event("custom_notify", message="Warnung: {} \n{}\nGefahr (0-4): {}".format(warning[9],start_end_readable,warning[0]), target="telegram_jo")
