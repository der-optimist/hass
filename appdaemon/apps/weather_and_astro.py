...
class weather_and_astro(hass.Hass):
    ...
    def initialize(self):
        ...
        time_load_meteogram = datetime.time(12, 56, 20) # update time for meteogram
    ...
    def load_dwd_warnings(self, kwargs):
        ...
        if r.status_code == 200:
            ...
            for warning in data_sorted:
                ...
                sensor_name = "sensor.dwd_warn_" + event.lower().replace(" ","_").replace("-","_").replace("ä","ae").replace("ö","oe").replace("ü","ue").replace("ß","ss") + "_" + warning[1].replace("-","_").replace(":","_").replace("T","_").replace("Z","")
