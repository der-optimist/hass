...
class weather_and_astro(hass.Hass):
    ...
    def initialize(self):
        ...
        self.run_every(self.load_dwd_warnings, datetime.datetime.now() + datetime.timedelta(seconds=5), 5 * 60) # update every 5 minutes
    ...
    def load_dwd_warnings(self, kwargs):
        ...
        if r.status_code == 200:
            ...
            for warning in data_sorted:
                ...
                sensor_name = "sensor.dwd_warn_" + str(abs(hash(event))) + "_" + warning[1].replace("-","").replace(":","").lower() + "_" + warning[2].replace("-","").replace(":","").lower()
    ...
    def datetime_readable(self, dt):
        ...
        minute_str = dt_local_naive_str[14:16]
        date_readable_str = self.date_to_text(dt_local_naive_str[0:10])
        dt_readable_str = date_readable_str + " " + hour_str + ":" + minute_str + " Uhr"
