...
class energy_consumption_daily(hass.Hass):
    ...
    def calculate_energy_consumption(self, date_str):
        ...
        if consumption_apparentpower_from_W > 0.0:
            cos_phi_day = consumption_power_from_W / consumption_apparentpower_from_W
        else:
            cos_phi_day = 0.0
        # compare effective
        measurement = "sensor.el_leistung_licht_sicherung_3_3"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        # compare apparent
        measurement = "sensor.el_leistung_licht_sicherung_3_3_scheinleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        consumption_kWh_from_kwh = counter_end - counter_start
        # apparent power
        measurement = "sensor.sonoff_pow_r2_2_scheinleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        consumption_apparentpower_from_W = consumption_Ws / 3600000
        # power
        measurement = "sensor.sonoff_pow_r2_2_wirkleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        if consumption_apparentpower_from_W > 0.0:
            ...
            cos_phi_day = 0.0
        # compare effective
        measurement = "sensor.el_leistung_licht_sicherung_3_2"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        consumption_compare_eff_from_W = consumption_Ws / 3600000
        # compare apparent
        measurement = "sensor.el_leistung_licht_sicherung_3_2_scheinleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        # energy meter
        measurement = "sensor.tasmota_sd_2_energie"
        query_start = 'SELECT last("{}") FROM "homeassistant_permanent"."autogen"."{}" WHERE time <= {}'.format(self.db_field, measurement, int(ts_start_local_ns))
        counter_start_generator = result_points = self.client.query(query_start).get_points()
        for point in counter_start_generator:
            counter_start = point["last"]
        ...
        consumption_kWh_from_kwh = counter_end - counter_start
        # apparent power
        measurement = "sensor.tasmota_sd_2_scheinleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        consumption_apparentpower_from_W = consumption_Ws / 3600000
        # power
        measurement = "sensor.tasmota_sd_2_wirkleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        # temporary section for power facor - TV
        # energy meter
        measurement = "sensor.tasmota_sd_3_energie"
        query_start = 'SELECT last("{}") FROM "homeassistant_permanent"."autogen"."{}" WHERE time <= {}'.format(self.db_field, measurement, int(ts_start_local_ns))
        counter_start_generator = result_points = self.client.query(query_start).get_points()
        for point in counter_start_generator:
            counter_start = point["last"]
        ...
        consumption_kWh_from_kwh = counter_end - counter_start
        # apparent power
        measurement = "sensor.tasmota_sd_3_scheinleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        consumption_apparentpower_from_W = consumption_Ws / 3600000
        # power
        measurement = "sensor.tasmota_sd_3_wirkleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        if consumption_apparentpower_from_W > 0.0:
            ...
            cos_phi_day = 0.0
        # compare effective
        measurement = "sensor.el_leistung_tv"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
        ...
        consumption_compare_eff_from_W = consumption_Ws / 3600000
        # compare apparent
        measurement = "sensor.el_leistung_tv_scheinleistung"
        query = 'SELECT "{}" FROM "homeassistant_permanent"."autogen"."{}" WHERE time >= {} AND time <= {} ORDER BY time DESC'.format(self.db_field, measurement, int(ts_start_local_ns_plus_buffer), int(ts_end_local_ns))
        #self.log(query)
        result_points = self.client.query(query).get_points()
        consumption_Ws = 0.0
